...
public class SimEngine {
    ...
    public void addLocation(String name, Location location) {
        ...
        if (locationMap.containsKey(name)) {
            throw new IllegalArgumentException("Location with the same name already exists: " + name);
        }
    ...
    public void addLocation(Location location) {
        String modelName = location.getName();
        addLocation(modelName, location);
    }
    ...
    public void printAllModels() {
        ...
        for (HashMap.Entry<String, Location> entry : locationMap.entrySet()) {
            String name = entry.getKey();
            Location location = entry.getValue();
            System.out.println("Name: " + name + ", Coordinate: " + location.toString());
        }
    }
    ...
    public void run() {
    	...
    	// while (!jobQueue.isEmpty() && currentTime <= endTime) {
    	while (currentTime <= endTime) {
    		...
    		for (AGV agv : AGVs) {
    			...
    			// 하던 일이 있다면 그 일을 계속 함
    				...
    				if (agv.isDest()) {
    					/* 필요 없는 코드 나중에 지울 예정
    					// Inventory에 도착하면 부품을 싣고 다음 경로 설정
    					if ( agv.getCurrentLocation().isSame(factory.getInvLoc()) ) {
    					...
    					// Storage에 도착하면 갖고 있는 부품을 창고에 내린 후 새로운 일을 찾는다.
    					else if ( agv.getCurrentLocation().isSame(factory.getStoLoc()) ) {
    						factory.putDownPart(agv);
    						agv.setAvailable(true);
    					}
    					else */
    					// 특정 포트에 부품을 가지고 도착하면 부품을 포트에 내린 후 새로운 일을 찾는다.
    					// TODO: 큐가 꽉차는 경우 고려 안 해서 수정 필요, 당장은 문제가 생기지 않을 것으로 예상.
